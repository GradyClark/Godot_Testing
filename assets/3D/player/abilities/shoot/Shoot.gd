...
onready var my_root = get_tree().get_root()
...
func spawn_impact():
	if $BulletSpread/RayCast.is_colliding():
		...
		var colliding_with = $BulletSpread/RayCast.get_collider()
		if  colliding_with is RigidBody:
		...
		if colliding_with.is_in_group("destroyable"):
			var destroyable: Node = colliding_with.get_node("destroyable")
			destroyable.change_health_by(-damage)
